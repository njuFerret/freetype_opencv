...
void testLowerAlpha(cvx::CvxFont &font, cv::Mat &img) {
  ...
  int fontSize = 32;
  cvx::putText(img, msg, cv::Point(40, 20), font, fontSize,
               cv::Scalar(0, 255, 0));
}
...
void testVerticalRotate(cvx::CvxFont &font, cv::Mat &img) {
  ...
  cvx::putText(img, msg5, cv::Point(700, 100), font, fontSize,
               randomColor(rng));
}
...
void testChinese() {
  cv::Mat img(400, 800, CV_8UC3,
              cv::Scalar(0, 0, 0));             // create a black background
  std::string fontPath = "./font/STCAIYUN.TTF"; // be careful to use the
  //    font that support Chinese
  // std::string fontPath = "C:/Windows/Fonts/simsun.ttc";
